'use strict';
// Adds GET/POST /attempts/:id/error-tags endpoints to app.js (additive only).
const fs = require('fs');
...
// Insert right before the "Save one answer into attempt_questions" block.
const anchor = '// ---------------------------------------------------------------------------' + EOL +
  '// Save one answer into attempt_questions (upsert).';
const idx = s.indexOf(anchor);
if (idx === -1) {
  console.error('ANCHOR NOT FOUND: saveAnswer block');
  process.exit(1);
}
const count = s.split(anchor).length - 1;
if (count !== 1) {
  console.error('EXPECTED 1 occurrence of saveAnswer anchor, got ' + count);
  process.exit(1);
}

s = s.slice(0, idx) + endpoints.replace(/\r?\n/g, EOL) + s.slice(idx);
if (s === orig) { console.error('no change'); process.exit(1); }
fs.writeFileSync(file, s, 'utf8');
console.log('OK: app.js endpoints added (', orig.length, '->', s.length, 'bytes )');
